...
class MenuViewController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        statusBarCover.frame = CGRectMake(0, 0, view.bounds.size.width, topLayoutGuide.length)
        
        gradientLayer.frame = CGRectMake(0, statusBarCover.frame.size.height, view.bounds.size.width, view.bounds.size.height - statusBarCover.frame.size.height)
    }
    ...
    @IBOutlet var statusBarCover: UIView!
                ...
                SharedDiningAPI().getMenu("ratty", date: NSDate()) { (let menusOpt, let errorOpt) -> () in
                    if arePositionsEqual(posAtStartOfLoad, self.currentPosition) {
                        if let mealMenus = menusOpt {
                            ...
                            // that's an error; just fallback and show breakfast (with an error msg):
                            self.currentPosition = (date: NSDate(), meal: Meal.Index(i: 0))
                        }
                    }
                }
    ...
    func updateCurrentPage(supressGradientAnimation: Bool = true) {
        ...
        let trim = colorForMeal(interpolatedMeal)
        navView.backgroundColor = trim
        statusBarCover.backgroundColor = color1
    }
